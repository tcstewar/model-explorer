...
def run(_filename,_iterations=1,**settings):
    ...
    f=None
    fname='.modextmp%08x.py'%random.randrange(0,0x70000000)
    ...
    for i in xrange(_iterations):
      for setting in make_settings_combinations(settings):
        ...
        if 'modex.log' not in core_code:
            core_code='import modex\nmodex.log()\n'+core_code

        code='%s\n%s'%(param_code,core_code)
        code=code.replace('\r\n','\n')
        logline='modex.log(data=True,screen=False,directory="%s/%s")'%(_filename[:-3],param_text)
        code=re.sub(r'modex\.log\([^)]*\)',logline,code)
